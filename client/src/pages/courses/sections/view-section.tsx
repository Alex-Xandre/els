...
const ViewModule = () => {
  ...
  const breadcrumbItems = [
    { label: "Home", href: "/" },
    {
      label: courses.find((x) => x._id === moduleId?.moduleId)?.title,
      href: `/moduleId?=${moduleId?.moduleId}`,
    },
    {
      label: module.title,
      href: `/moduleId?=${module._id}`,
      isCurrentPage: true,
    },
  ];
  ...
  const renderItems = useCallback(() => {
    if (currentId && !currentId?.sectionType) {
      ...
      const { title, description, category, assesmentDueDate, questions } =
        activity;

      const totalPoints = questions.reduce(
        (acc, question) => acc + question.questionPoints,
        0
      );
          ...
          <div className=" flex-1 rounded-md py-3 mt-4 flex flex-col gap-y-2">
            <ul className="border-b inline-flex text-sm z-50 -ml-2">
              <li
                ...
                className={`px-2 ${
                  isInstruction ? " border-b-[2px] border-primary" : ""
                }`}
              >
                Instructions
              </li>
            ...
            {isInstruction && (
              <>
                ...
                {(currentId as AssesmentType)?.attempts >
                  submissions.length && (
                  <Button
  ...
  const handleNext = () => {
    if (currentIndex < organizeSection.length - 1) {
      ...
      const nextItem =
        sections.find((x) => x._id === nextId) ||
        activity.find((x) => x._id === nextId);

      // Check if next item is unlocked
      const isUnlocked =
        nextItem?.isUnlock ||
        progress?.[0]?.completedLessons?.includes(nextItem?._id);
  ...
  const handlePrevious = () => {
    if (currentIndex > 0) {
      ...
      const prevItem =
        sections.find((x) => x._id === prevId) ||
        activity.find((x) => x._id === prevId);

      // Disable previous button if at the start (index 0)
      const isUnlocked =
        currentIndex === 0 ||
        progress?.includes(organizeSection[currentIndex - 1]) ||
        prevItem?.isUnlock;
  ...
  useEffect(() => {
    ...
    if (current) {
      ...
      const findAssessment: AssesmentType = activity.find(
        (x) => x._id === current
      );
      ...
      <NavContainer>
        ...
        {user.role === "admin" && (
          <div className="inline-flex gap-x-2">
            <Button onClick={() => navigate(`/${myParamValue}/new-assesment`)}>
              Add Activity
            </Button>
            <Button
              variant="secondary"
      ...
      {currentId ? (
        <>
          <header className="inline-flex items-center justify-between w-full mt-5">
            <Button
              className={
                currentIndex === 0 ? "cursor-not-allowed opacity-35" : ""
              }
            ...
            <Button
              className={
                currentIndex === organizeSection.length - 1 &&
                currentIndex !== 0
        ...
        <>
          ...
          {organizeSection.map((item, index) => {
            const findSection: SectionTypes = sections.find(
              (x) => x._id === item && x.isDeleted === false
            );
            const findAssessment: AssesmentType = activity.find(
              (x) => x._id === item && x.isDeleted === false
            );
            ...
            const isUnlocked =
              ...
              (prevItem &&
                ...
                progress?.[0]?.completedLessons.includes(findSection?._id)) ||
              ...
              return (
                <ActivityCard
                  ...
                  setCurrentId={() => {
                    ...
                    navigate(
                      `${location.pathname}${location.search}&current=${findSection._id}`
                    );
                  }}
                />
              );
