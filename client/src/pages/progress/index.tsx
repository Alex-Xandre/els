            ...
            <span className="flex flex-col">
              {value != "-" ? (
                <span
                  ...
                  onClick={() =>
                    nav(
                      `/${myParamValue}/submissions/view?=${value._id}&examId=${value.activityId}`
                    )
                  }
                >
                  View Submission
      ...
      <ReusableTable
        ...
        data={
          selectedUser === null
            ...
            : activity.map((item) => {
                //overall progress
                const studentProgress = submissions.filter(
                  (x) => x.activityId === item._id && x.user === selectedUser
                );

                const getLatestSub =
                  studentProgress.length > 0
                    ? studentProgress[studentProgress.length - 1]
                    : undefined;

                const totalScore = item.questions.reduce(
                  (score, question) => score + question.questionPoints,
                  0
                );
