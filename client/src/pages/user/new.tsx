...
const NewUser = () => {
  ...
  useEffect(() => {
    if (state?.isEdit) {
      ...
      const items = allUser.filter((x) => x.isDeleted === false).find((x) => x._id === myParamValue) as UserTypes;
      if (!items) return;

      setUserData({
        ...items,
        personalData: items.personalData ?? userData.personalData,
      });
    }
    ...
    try {
      const res = await registerUserByAdmin({
        ...
        password:
          userData.password !== ""
            ? userData.password
            : new Date(userData?.personalData?.birthday as string)
                .toISOString()
                .split("T")[0],
      });
    ...
    <Container>
      ...
      <section className="h-[calc(100vh-100px)] pb-24 overflow-y-auto">
        <FormContainer title="Account Information">
          <img
            src={
              userData.profile ? userData.profile : "https://placehold.co/400"
            }
          ...
          {accountForm.map((items) => (
            <div key={items.name} className="lg:w-full">
              ...
              {items.type === "option" ? (
                ...
                <Input
                  ...
                  onChange={
                    items.type === "file" ? onFileChange : onInputChange
                  }
                  value={
                    items.type === "file"
                      ? undefined
                      : (userData[items.name] as keyof UserTypes as string)
                  }
                />
              )}
            </div>
        ...
        <FormContainer title="Personal Information">
          {personalForm.map((items) => (
            <div key={items.name} className="lg:w-[49%]">
              <Label> {items.label}</Label>
